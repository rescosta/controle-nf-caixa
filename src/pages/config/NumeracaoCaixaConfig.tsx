...
export default function NumeracaoCaixaConfig() {
  const [valor, setValor] = useState<number>(1)
  const [saved, setSaved] = useState(false)

  useEffect(() => {
    api.settings.get('seq_inicial_caixa').then((v: string | null) => {
      if (v) setValor(parseInt(v, 10))
    })
  }, [])

  async function handleSalvar() {
    await api.settings.set('seq_inicial_caixa', String(valor))
    setSaved(true)
  ...
  return (
    <div className="p-6 max-w-lg">
      <h2 className="text-lg font-semibold text-slate-100 mb-1">Numeração dos Caixas</h2>
      <p className="text-sm text-slate-400 mb-6">
        Define o número inicial da sequência. O sistema usa sempre o maior entre este valor e o
        último número já utilizado — alterar este campo não afeta caixas já existentes.
      </p>

      <div className="bg-slate-800 rounded-lg p-5 border border-slate-700 space-y-4">
        <div>
          <label className="block text-sm text-slate-300 mb-1">Número inicial (seq_inicial_caixa)</label>
          <input
            type="number"
            min={1}
            max={9999}
